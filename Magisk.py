...
def yecho(info):
    print(f"\033[36m[{strftime('%H:%M:%S')}]{info}\033[0m")


class Magisk_patch:
    def __init__(self, boot_img, Magisk_dir, IS64BIT=True, KEEPVERITY=False, KEEPFORCEENCRYPT=False, RECOVERYMODE=False, MAGISAPK=None, PATCH_ARCH=None):
        self.SKIPBACKUP = ''
        self.SKIPSTUB = ''
        self.SKIP64 = ''
        self.SKIP32 = ''
    ...
    def exec(self, *args, out=0):
        ...
        try:
            ret = subprocess.Popen(full, shell=False, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, creationflags=conf)
    ...
    def patch_kernel(self):
        if os.path.exists(os.path.join(local, 'kernel')):
            self.exec('hexpatch', 'kernel', '49010054011440B93FA00F71E9000054010840B93FA00F7189000054001840B91FA00F7188010054', 'A1020054011440B93FA00F7140020054010840B93FA00F71E0010054001840B91FA00F7181010054')
    ...
    def get_arch(self):
        with zipfile.ZipFile(self.MAGISKAPK) as ma:
            return [i.split('/')[1].strip() for i in ma.namelist() if i.startswith('lib') and i.endswith('libmagiskboot.so')]
