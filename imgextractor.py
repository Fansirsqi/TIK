...
class Extractor:
        ...
        def scan_dir(root_inode, root_path=""):
            for entry_name, entry_inode_idx, entry_type in root_inode.open_dir():
                ...
                if tmp_path.find(" ", 1, len(tmp_path)) > 0:
                    ...
                    self.fs_config.append(f"{tmp_path.replace(' ', '_')} {uid} {gid} {mode}{cap} {link_target}")
                else:
                    self.fs_config.append(f"{tmp_path} {uid} {gid} {mode}{cap} {link_target}")
    ...
    def main(self, target: str, output_dir: str, work: str, target_type: str = "img"):
        self.EXTRACT_DIR = os.path.realpath(os.path.dirname(output_dir)) + os.sep + self.__out_name(os.path.basename(output_dir))
