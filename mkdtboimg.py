...
class DtEntry(object):
    ...
    REQUIRED_KEYS_V0 = ("dt_file", "dt_size", "dt_offset", "id", "rev", "custom0", "custom1", "custom2", "custom3")
    REQUIRED_KEYS_V1 = ("dt_file", "dt_size", "dt_offset", "id", "rev", "flags", "custom0", "custom1", "custom2")
    ...
    def __init__(self, **kwargs):
        ...
        if missing_keys:
            raise ValueError("Missing keys in DtEntry constructor: %r" % sorted(missing_keys))
    ...
    def __str__(self):
        sb = ["{key:>20} = {value:d}".format(key="dt_size", value=self.__dt_size), "{key:>20} = {value:d}".format(key="dt_offset", value=self.__dt_offset), "{key:>20} = {value:08x}".format(key="id", value=self.__id), "{key:>20} = {value:08x}".format(key="rev", value=self.__rev)]
        if self.__version == 1:
            sb.append("{key:>20} = {value:08x}".format(key="flags", value=self.__flags))
        sb.append("{key:>20} = {value:08x}".format(key="custom[0]", value=self.__custom0))
        sb.append("{key:>20} = {value:08x}".format(key="custom[1]", value=self.__custom1))
        sb.append("{key:>20} = {value:08x}".format(key="custom[2]", value=self.__custom2))
        if self.__version == 0:
            sb.append("{key:>20} = {value:08x}".format(key="custom[3]", value=self.__custom3))
        return "\n".join(sb)
...
class Dtbo(object):
    ...
    def _update_dt_table_header(self):
        ...
        metadata buffer.
        """
        struct.pack_into(">8I", self.__metadata, 0, self.magic, self.total_size, self.header_size, self.dt_entry_size, self.dt_entry_count, self.dt_entries_offset, self.page_size, self.version)
    ...
    def _update_dt_entry_header(self, dt_entry, metadata_offset):
        ...
        """
        if self.version == 0:
            struct.pack_into(">8I", self.__metadata, metadata_offset, dt_entry.size, dt_entry.dt_offset, dt_entry.image_id, dt_entry.rev, dt_entry.custom0, dt_entry.custom1, dt_entry.custom2, dt_entry.custom3)
        elif self.version == 1:
            struct.pack_into(">8I", self.__metadata, metadata_offset, dt_entry.size, dt_entry.dt_offset, dt_entry.image_id, dt_entry.rev, dt_entry.flags, dt_entry.custom0, dt_entry.custom1, dt_entry.custom2)
    ...
    def _read_dtbo_image(self):
        ...
        self.__metadata_size = self.header_size + self.dt_entry_count * self.dt_entry_size
        if file_size < self.__metadata_size:
            raise ValueError("Invalid or truncated DTBO file of size %d expected %d" % file_size, self.__metadata_size)

        num_ints = self._DT_TABLE_HEADER_INTS + self.dt_entry_count * self._DT_ENTRY_HEADER_INTS
        ...
        self.__file.seek(0)
        self.__metadata = struct.unpack(format_str, self.__file.read(self.__metadata_size))
    ...
    def __str__(self):
        sb = ["dt_table_header:"]
        _keys = ("magic", "total_size", "header_size", "dt_entry_size", "dt_entry_count", "dt_entries_offset", "page_size", "version")
        for key in _keys:
            if key == "magic":
                sb.append("{key:>20} = {value:08x}".format(key=key, value=self.__dict__[key]))
            else:
                sb.append("{key:>20} = {value:d}".format(key=key, value=self.__dict__[key]))
        count = 0
        for dt_entry in self.__dt_entries:
    ...
    def compress_dt_entry(self, compression_format, dt_entry_file):
        ...
        compress_gzip = zlib.compressobj(zlib.Z_DEFAULT_COMPRESSION, zlib.DEFLATED, self._GZIP_COMPRESSION_WBITS)  # gzip
    ...
    def add_dt_entries(self, dt_entries):
        ...
        for dt_entry in dt_entries:
            ...
            if entry and (entry.compression_info() == dt_entry_compression_info):
                ...
                compressed_entry, dt_entry.size = self.compress_dt_entry(dt_entry_compression_info, dt_entry.dt_file)
    ...
    def extract_dt_file(self, idx, fout, decompress):
        ...
        if decompress and compression_format:
            if compression_format == CompressionFormat.ZLIB_COMPRESSION or compression_format == CompressionFormat.GZIP_COMPRESSION:
...
def parse_dt_entry(global_args, arglist):
    ...
    parser.add_argument("dt_file", nargs="?", type=argparse.FileType("rb"), default=None)
    parser.add_argument("--id", type=str, dest="id", action="store", default=global_args.id)
    parser.add_argument("--rev", type=str, dest="rev", action="store", default=global_args.rev)
    parser.add_argument("--flags", type=str, dest="flags", action="store", default=global_args.flags)
    parser.add_argument("--custom0", type=str, dest="custom0", action="store", default=global_args.custom0)
    parser.add_argument("--custom1", type=str, dest="custom1", action="store", default=global_args.custom1)
    parser.add_argument("--custom2", type=str, dest="custom2", action="store", default=global_args.custom2)
    parser.add_argument("--custom3", type=str, dest="custom3", action="store", default=global_args.custom3)
...
def create_dtbo_image(fout, list, page_size=2048, version=0, dt_type="dtb", id="0", rev="0", flags="0", custom0="0", custom1="0", custom2="0", custom3="0"):
    ...
    data = argparse.Namespace(id=id, rev=rev, flags=flags, custom0=custom0, custom1=custom1, custom2=custom2, custom3=custom3, version=version)
