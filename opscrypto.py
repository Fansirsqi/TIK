...
class QCSparse:
    ...
    def get_chunk_size(self):
        if self.total_blks < self.offset:
            self.error('The header said we should have %u output blocks, but we saw %u' % (self.total_blks, self.offset))
            return -1
        ...
        if chunk_type == 0xCAC1:
            if data_sz != (chunk_sz * self.blk_sz):
                self.error('Raw chunk input size (%u) does not match output size (%u)' % (data_sz, chunk_sz * self.blk_sz))
                return -1
            else:
...
def key_update(iv1, asbox):
    ...
    e = gsbox(((b >> 0x10) & 0xFF) * 8 + 2) ^ gsbox(((a >> 8) & 0xFF) * 8 + 3) ^ gsbox((c >> 0x18) * 8 + 1) ^ gsbox((d & 0xFF) * 8) ^ asbox[4]  # 35C2A10B

    h = gsbox(((c >> 0x10) & 0xFF) * 8 + 2) ^ gsbox(((b >> 8) & 0xFF) * 8 + 3) ^ gsbox((d >> 0x18) * 8 + 1) ^ gsbox((a & 0xFF) * 8) ^ asbox[5]  # 75CF3118
    i = gsbox(((d >> 0x10) & 0xFF) * 8 + 2) ^ gsbox(((c >> 8) & 0xFF) * 8 + 3) ^ gsbox((a >> 0x18) * 8 + 1) ^ gsbox((b & 0xFF) * 8) ^ asbox[6]  # 6AD3F5C4
    a = gsbox(((d >> 8) & 0xFF) * 8 + 3) ^ gsbox(((a >> 0x10) & 0xFF) * 8 + 2) ^ gsbox((b >> 0x18) * 8 + 1) ^ gsbox((c & 0xFF) * 8) ^ asbox[7]  # D99AC8FB
    ...
    for f in range(asbox[0x3C] - 2):
        ...
        l = i >> 0x18
        t = e >> 8
        e = gsbox(((i >> 0x10) & 0xFF) * 8 + 2) ^ gsbox(((h >> 8) & 0xFF) * 8 + 3) ^ gsbox((a >> 0x18) * 8 + 1) ^ gsbox((e & 0xFF) * 8) ^ asbox[g]  # B67F2106, 82508918
        h = gsbox(((a >> 0x10) & 0xFF) * 8 + 2) ^ gsbox(((i >> 8) & 0xFF) * 8 + 3) ^ gsbox(d * 8 + 1) ^ gsbox((h & 0xFF) * 8) ^ asbox[g + 1]  # 85813F52
        i = gsbox((z & 0xFF) * 8 + 2) ^ gsbox(((a >> 8) & 0xFF) * 8 + 3) ^ gsbox(s * 8 + 1) ^ gsbox((i & 0xFF) * 8) ^ asbox[g + 2]  # C8022573
        a = gsbox((t & 0xFF) * 8 + 3) ^ gsbox((m & 0xFF) * 8 + 2) ^ gsbox(l * 8 + 1) ^ gsbox((a & 0xFF) * 8) ^ asbox[g + 3]  # AD34EC55
        g = g + 4
    # a=6DB8AA0E
    # b=ABD51D58
    # c=AFCBAFFF
    ...
    # t=510805
    return [
        (gsbox(((i >> 0x10) & 0xFF) * 8) & 0xFF0000) ^ (gsbox(((h >> 8) & 0xFF) * 8 + 1) & 0xFF00) ^ (gsbox((a >> 0x18) * 8 + 3) & 0xFF000000) ^ gsbox((e & 0xFF) * 8 + 2) & 0xFF ^ asbox[g],
        (gsbox(((a >> 0x10) & 0xFF) * 8) & 0xFF0000) ^ (gsbox(((i >> 8) & 0xFF) * 8 + 1) & 0xFF00) ^ (gsbox((e >> 0x18) * 8 + 3) & 0xFF000000) ^ (gsbox((h & 0xFF) * 8 + 2) & 0xFF) ^ asbox[g + 3],
        (gsbox(((e >> 0x10) & 0xFF) * 8) & 0xFF0000) ^ (gsbox(((a >> 8) & 0xFF) * 8 + 1) & 0xFF00) ^ (gsbox((h >> 0x18) * 8 + 3) & 0xFF000000) ^ (gsbox((i & 0xFF) * 8 + 2) & 0xFF) ^ asbox[g + 2],
        (gsbox(((h >> 0x10) & 0xFF) * 8) & 0xFF0000) ^ (gsbox(((e >> 8) & 0xFF) * 8 + 1) & 0xFF00) ^ (gsbox((i >> 0x18) * 8 + 3) & 0xFF000000) ^ (gsbox((a & 0xFF) * 8 + 2) & 0xFF) ^ asbox[g + 1],
    ]
