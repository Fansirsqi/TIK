...
def call(exe, kz="Y", out=0, shstate=False, sp=0):
    ...
    if os.name != "posix":
        ...
        conf = 0
    try:
        ret = subprocess.Popen(cmd, shell=shstate, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, creationflags=conf)
...
class dbkxyt:
    def __init__(self, work, code, extra):
        ...
        with open(dir_ + "META-INF" + os.sep + "com" + os.sep + "google" + os.sep + "android" + os.sep + "update-binary", "r+", encoding="utf-8", newline="\n") as script:
            ...
            for t in os.listdir(dir_ + "images"):
                if t.endswith(".img") and not os.path.isdir(dir_ + t):
                    ...
                    if os.path.getsize(os.path.join(dir_ + "images", t)) > 209715200:
                        ...
                        lines.insert(add_line, 'package_extract_zstd "images/{}.zst" "/dev/block/by-name/{}"\n'.format(t, t[:-4]))
                    else:
...
def img2sdat(input_image, out_dir=".", version=None, prefix="system"):
    ...
    blockimgdiff.BlockImageDiff(sparse_img.SparseImage(input_image, tempfile.mkstemp()[1], "0"), None, version).Compute(out_dir + "/" + prefix)
...
class LOGODUMPER(object):
    ...
    def chkimg(self, img: str):
        ...
        with open(img, "rb") as f:
            ...
            self.magic = struct.unpack(self.structstr, f.read(struct.calcsize(self.structstr)))[0]
            while True:
