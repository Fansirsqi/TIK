...
def error(code, message):
    ...
    table.add_section()
    table.add_row('[green]Report:https://github.com/ColdWindScholar/TIK/issues')
    Console().print(table)
    input()
    sys.exit(1)
...
try:
    if os.path.basename(sys.argv[0]) == f'run_new{str() if os.name == "posix" else ".exe"}':
        ...
        shutil.copyfile(os.path.join(LOCALDIR, f'run_new{str() if os.name == "posix" else ".exe"}'), os.path.join(LOCALDIR, f'run{str() if os.name == "posix" else ".exe"}'))
...
class upgrade:
    ...
    def __init__(self):
        if not os.path.exists(temp):
            os.makedirs(temp)
        cls()
        with Console().status('[blue]正在检测新版本...[/]'):
            try:
        ...
        if not data:
            ...
            if data.get('version', settings.version) != settings.version:
                ...
                print(f"\033[0;32;40m发现新版本：\033[0m\033[0;36;40m{settings.version} --> {data.get('version')}\033[0m")
                ...
                if input('\033[0;33;40m是否更新?[1/0]\033[0m') == '1':
                    ...
                    shutil.move(os.path.join(extract_path, f'run{str() if os.name == "posix" else ".exe"}'), os.path.join(LOCALDIR, f'run_new{str() if os.name == "posix" else ".exe"}'))
...
class setting:
    ...
    def settings1(self):
        actions = {
            '1': lambda: settings.change('brcom', brcom if (brcom := input('  调整brotli压缩等级(整数1-9，级别越高，压缩率越大，耗时越长):')).isdigit() and 0 < int(brcom) < 10 else '1'),
            '2': lambda: settings.change('diysize', '1' if input('  打包Ext镜像大小[1]动态最小 [2]原大小:') == '2' else ''),
            '3': lambda: settings.change('pack_e2', '0' if input('  打包方案: [1]make_ext4fs [2]mke2fs+e2fsdroid:') == '1' else '1'),
            '6': lambda: settings.change('pack_sparse', '1' if input('  Img是否打包为sparse(压缩体积)[1/0]\n  请输入序号:') == '1' else '0'),
            '7': lambda: settings.change('diyimgtype', '1' if input('  打包镜像格式[1]同解包格式 [2]可选择:') == '2' else ''),
            '8': lambda: settings.change('erofs_old_kernel', '1' if input('  EROFS打包是否支持旧内核[1/0]') == '1' else '0'),
        }
        cls()
        print(
            f"""
    ...
    def settings2(self):
        cls()
        actions = {
            '1': lambda: settings.change('super_group', super_group if (super_group := input('  请输入（无特殊字符）:')) else 'qti_dynamic_partitions'),
            '2': lambda: settings.change('metadatasize', metadatasize if (metadatasize := input('  设置metadata最大保留size(默认为65536，至少512):')) else '65536'),
            '3': lambda: settings.change('BLOCKSIZE', BLOCKSIZE if (BLOCKSIZE := input(f'  分区打包扇区/块大小：{settings.BLOCKSIZE}\n  请输入: ')) else '4096'),
    ...
    \033[33m  > 工具设置 \033[0m\n
       ...
       --------------------------
        ...
        if op_pro == '0':
            ...
            print('  首页banner: [1]TIK4 [2]镰刀斧头 [3]TIK2 [4]原神 [5]DXY [6]None')
    ...
    def settings4():
        cls()
        print(f'\033[31m {banner.banner5} \033[0m')
        print('\033[96m 开源的安卓全版本ROM处理工具\033[0m')
        print('\033[31m---------------------------------\033[0m')
        print('\033[93m作者:\033[0m \033[92mColdWindScholar\033[0m')
        print('\033[93m开源地址:\033[0m \033[91mhttps://github.com/ColdWindScholar/TIK\033[0m')
        print('\033[93m软件版本:\033[0m \033[44mAlpha Edition\033[0m')
        print('\033[93m开源协议:\033[0m \033[68mGNU General Public License v3.0 \033[0m')
        print('\033[31m---------------------------------\033[0m')
        print('\033[93m特别鸣谢:\033[0m')
        print('\033[94mAffggh')
        print('Yeliqin666')
        print('YukongA')
        print('\033[0m')
...
class Tool:
    ...
    def main(self):
        ...
        if settings.banner != '6':
            ...
            print('=' * 50)
        print(f'\033[1;37;43m Version \033[1;92;44m Alpha Edition \033[1;37;42m Version{settings.version} \033[0m\n')
        if settings.online == 'true':
            try:
                ...
                print('\033[36m “开源，是一场无问西东的前行”\033[0m\n')
            else:
                print(f'\033[36m “{shiju}” \n ---{another} 《{fr}》\033[0m\n')

        else:
            print('\033[36m “开源，是一场无问西东的前行”')

        print(' >\033[33m 项目列表 \033[0m\n')
        print('\033[31m   [00]  删除项目\033[0m\n\n', '  [0]  新建项目\n')

        for pros in os.listdir(LOCALDIR):
            if pros in ['bin', '__pycache__'] or pros.startswith('.'):
                continue
    ...
    def project(self):
        ...
        print(' \n\033[31m>项目菜单 \033[0m\n')
        print(f'  项目：{self.pro}\033[91m(不完整)\033[0m\n') if not os.path.exists(os.path.abspath('config')) else print(f'  项目：{self.pro}\n')
            ...
            for root, dirs, files in os.walk(project):
                for f in files:
                    if f.endswith('.br') or f.endswith('.dat') or f.endswith('.list'):
                        if not os.path.isfile(os.path.join(project, 'TI_out' + os.sep + f)) and os.access(os.path.join(project, f), os.F_OK):
...
def get_all_file_paths(directory) -> Any:
    # 初始化文件路径列表
    ...
    if op_pro == '66':
        ...
        if int(op_pro) in mysubs.keys():
            ...
            if os.path.exists(plugin_path + os.sep + 'main.sh'):
                ...
                call(f'busybox ash {gen} {os.path.join(plugin_path, "main.sh").replace(os.sep, "/")}')
                f_remove(gen)
            else:
...
class installmpk:
    ...
    def install(self):
        ...
        try:
            ...
            depends = ''
        minfo = {
            'name': '%s' % (self.mconf.get('module', 'name')),
        ...
        with open(binner + os.sep + 'subs' + os.sep + self.mconf.get('module', 'identifier') + os.sep + 'info.json', 'w') as f:
...
def dboot(infile, orig):
    ...
    if os.path.isdir(infile + os.sep + 'ramdisk'):
        try:
            ...
            return
        cpio = utils.findfile('cpio.exe' if os.name != 'posix' else 'cpio', ebinner).replace('\\', '/')
...
def unpackboot(file, project):
    ...
    if os.access(project + os.sep + name + os.sep + 'ramdisk.cpio', os.F_OK):
        ...
        with open(project + os.sep + name + os.sep + 'comp', 'w') as f:
            f.write(comp)
        if comp != 'unknow':
            os.rename(project + os.sep + name + os.sep + 'ramdisk.cpio', project + os.sep + name + os.sep + 'ramdisk.cpio.comp')
            if call('magiskboot decompress %s %s' % (project + os.sep + name + os.sep + 'ramdisk.cpio.comp', project + os.sep + name + os.sep + 'ramdisk.cpio')) != 0:
...
def makedtb(sf, project):
    ...
    for dts_files in os.listdir(dtbdir + os.sep + 'dts_files'):
        ...
        if call(f'dtc -@ -I "dts" -O "dtb" "{dtb_}" -o "{dtbdir + os.sep}new_dtb_files{os.sep}{new_dtb_files}.dtb"', out=1) != 0:
            ywarn('回编译dtb失败')
...
def undtbo(project, infile):
    ...
    for dtbo_files in os.listdir(dtbodir + os.sep + 'dtbo_files'):
        if dtbo_files.startswith('dtbo.'):
            ...
            if call(f'dtc -@ -I "dtb" -O "dts" {dtbofiles} -o "{dtbodir + os.sep + "dts_files" + os.sep + dts_files}"', out=1) != 0:
...
def makedtbo(sf, project):
    ...
    for dts_files in os.listdir(dtbodir + os.sep + 'dts_files'):
        ...
        call(f'dtc -@ -I "dts" -O "dtb" {dtb_} -o {dtbodir + os.sep + "new_dtbo_files" + os.sep + new_dtbo_files}', out=1)
    yecho('正在生成dtbo.img...')
    list_ = []
...
def inpacker(name, project, form, ftype, json_=None):
    ...
    if ftype == 'erofs':
        ...
        call(f'mkfs.erofs {other_} -z{settings.erofslim}  -T {utc} --mount-point=/{name} --fs-config-file={fs_config} --product-out={os.path.dirname(out_img)} --file-contexts={file_contexts} {out_img} {in_files}')
    else:
        if settings.pack_e2 == '0':
            call(f'make_ext4fs -J -T {utc} -S {file_contexts} -l {img_size0} -C {fs_config} -L {name} -a {name} {out_img} {in_files}')
        else:
            call(f'mke2fs -O ^has_journal -L {name} -I 256 -M /{name} -m 0 -t ext4 -b {settings.BLOCKSIZE} {out_img} {size}')
            call(f'e2fsdroid -e -T {utc} -S {file_contexts} -C {fs_config} -a /{name} -f {in_files} {out_img}')
...
def packsuper(project):
    ...
    insuper(project + os.sep + 'super', project + os.sep + 'TI_out' + os.sep + 'super.img', supersize, supertype, ifsparse)
    ...
    for imag in os.listdir(Imgdir):
        if imag.endswith('.img'):
            ...
            if f'partition {image}:readonly' not in superpa and f'partition {image}_a:readonly' not in superpa:
                print(f'待打包分区:{image}')
                if stype in ['VAB', 'AB']:
                    if os.path.isfile(Imgdir + os.sep + image + '_a.img') and os.path.isfile(Imgdir + os.sep + image + '_b.img'):
...
def packpayload(project):
    ...
    tool_auto_size = sum([os.path.getsize(os.path.join(project + os.sep + 'payload', p)) for p in os.listdir(project + os.sep + 'payload') if os.path.isfile(os.path.join(project + os.sep + 'payload', p))]) + 409600
    tool_auto_size = versize(tool_auto_size)
...
def inpayload(supersize, project):
    ...
    with open(project + os.sep + 'payload' + os.sep + 'parts_info.txt', 'w', encoding='utf-8', newline='\n') as txt:
    ...
    call(f"delta_generator --out_file={out} {inparts} --dynamic_partition_info_file={os.path.join(project, 'payload', 'parts_info.txt')}")
...
def unpack(file, info, project):
    ...
    if info == 'sparse':
        ...
        filepath = str(os.path.dirname(file))
        version = utils.sdat2img(os.path.join(filepath, partname + '.transfer.list'), os.path.join(filepath, partname + '.new.dat'), os.path.join(filepath, partname + '.img')).version
        ...
        args = {'decrypt': True, '<filename>': file, 'outdir': os.path.join(project, os.path.dirname(file).split('.')[0])}
        opscrypto.main(args)
    elif info == 'payload':
        ...
        for fd in [f for f in os.listdir(project) if re.search(r'\.win\d+', f)]:
            with open(project + os.path.basename(fd).rsplit('.', 1)[0], 'ab') as ofd:
                for fd1 in sorted([f for f in os.listdir(project) if f.startswith(os.path.basename(fd).rsplit('.', 1)[0] + '.')], key=lambda x: int(x.rsplit('.')[3])):
        ...
        filepath = str(os.path.dirname(file))
        utils.sdat2img(os.path.join(filepath, partname + '.transfer.list'), os.path.join(filepath, partname + '.new.dat'), os.path.join(filepath, partname + '.img'))
        unpack(os.path.join(filepath, partname + '.img'), gettype(os.path.join(filepath, partname + '.img')), project)
    elif info == 'erofs':
